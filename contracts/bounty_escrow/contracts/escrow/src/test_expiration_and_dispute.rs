...
use crate::{BountyEscrowContract, BountyEscrowContractClient, DisputeOutcome, DisputeReason, Error, EscrowStatus};
use soroban_sdk::{
...
fn test_pending_claim_blocks_refund() {
    ...
    setup.escrow.authorize_claim(&bounty_id, &setup.contributor, &DisputeReason::QualityIssue);
...
fn test_multiple_bounties_independent_resolution() {
    ...
    setup.escrow.authorize_claim(&1, &setup.contributor, &DisputeReason::Other);
    ...
    setup.escrow.authorize_claim(&3, &setup.contributor, &DisputeReason::Other);

    setup.env.ledger().set_timestamp(now + 550);

    setup.escrow.cancel_pending_claim(&1, &DisputeOutcome::CancelledByAdmin);
...
fn test_expiry_does_not_bypass_active_dispute() {
    ...
    s.escrow.authorize_claim(&bounty_id, &s.contributor, &DisputeReason::Other);
...
fn test_dispute_before_expiry_cancel_then_refund_after_deadline() {
    ...
    s.escrow.cancel_pending_claim(&bounty_id, &DisputeOutcome::CancelledByAdmin);
